...
> Repeat (usually anywhere between1-10){<br>   for i = 1:m{<br>       $\theta:={\theta}_{j}-\alpha\left( {h}_{\theta}\left({x}^{(i)}\right)-{y}^{(i)} \right){{x}_{j}}^{(i)}$<br>       (for j=0:n)<br>    }<br>}
...
> Repeat {<br>   for i = 1:m{<br>       $\theta:={\theta}_{j}-\alpha\frac{1}{b}\sum_\limits{k=i}^{i+b-1}\left( {h}_{\theta}\left({x}^{(k)}\right)-{y}^{(k)} \right){{x}_{j}}^{(k)}$<br>       (for j=0:n)<br>       i +=10  <br>   }<br>}
...
> Repeat forever (as long as the website is running) {<br> Get $\left(x,y\right)$ corresponding to the current user <br>       $\theta:={\theta}_{j}-\alpha\left( {h}_{\theta}\left({x}\right)-{y} \right){{x}_{j}}$<br>       (for j=0:n) <br>   }
